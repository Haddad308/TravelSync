...
async function getReservation(SetReservation, setIsLoading, status = "", id = "") {
    setIsLoading(true);
    let data = await instance
        .get(`/api/reservations${status ? `?filters=${encodeURIComponent(JSON.stringify({ status: status }))}` : ""}${id ? `/${id}` : ""}`, {
            headers: {
