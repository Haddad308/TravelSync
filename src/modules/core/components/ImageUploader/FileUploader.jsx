...
export default function FileUploader({ imageName, index, files, setFiles }) {
    ...
    return (
        <div id="Uploaded Image" className="flex items-center justify-between  w-4/5 p-3 border-2  border-[#11181c] rounded-md">
            <div className="flex items-center gap-2">
                <BsFileEarmarkImage className="w-5 h-5" />
                <p >{imageName.slice(0, 25)} {imageName.length > 25 ? "...." : ""}</p>
            </div>
            <IoMdClose className="w-5 h-5 self-end cursor-pointer" onClick={() => { deleteFile(files, setFiles, index) }} />
        </div>
    )
}
